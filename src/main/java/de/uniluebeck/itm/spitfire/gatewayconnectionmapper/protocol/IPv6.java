...
/**
 * This class represents a IPv6 packet and if it carries UDP or TCP payload also
...
public class IPv6 {
     ...
     */
    public IPv6(byte[] data) throws Exception {
     ...
     * @throws Exception Will be thrown when decoding fails
     */
    public IPv6(byte[] buffer, int nBytes) throws Exception {
   ...
   /**
     ...
     */
    private static byte[] calculateTCPchecksum(IPv6 p) throws Exception {
        if (!p.isTCP()) {
...
//        System.out.println(compareBytes(p2.encode(), new IPv6(p2.encode()).encode()));
     ...
     * @throws Exception Will be thrown when calculation fails
     */
    private static byte[] calculateUDPchecksum(IPv6 p) throws Exception {
        if (!p.isUDP()) {
